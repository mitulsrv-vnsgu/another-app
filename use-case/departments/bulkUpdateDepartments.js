...
const bulkUpdateDepartments = ({ departmentsDb }) => async (params,req,res) => {
  ...
  let checkUniqueFields = await checkUniqueFieldsInDatabase([ 'name', 'code', 'email' ],params.dataToUpdate,'BULK_UPDATE',params.query);
  ...
  const updatedDepartments = await departmentsDb.updateMany(params.query,params.dataToUpdate);
