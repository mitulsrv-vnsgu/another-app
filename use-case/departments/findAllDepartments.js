...
const findAllDepartments = ({
  ...
  if (isCountOnly){
    let totalRecords = await departmentsDb.count(query);
    return response.success({ data: { totalRecords } });  
  }
  else {
    let foundDepartments = await departmentsDb.paginate(query,options);
    if (!foundDepartments){
...
module.exports = findAllDepartments;