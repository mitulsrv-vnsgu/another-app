...
const softDeletePatient = async (req,res) => {
  try {
    ...
    let updatedPatient = await dbService.updateOne(Patient, query, updateBody);
    if (!updatedPatient){
      return res.recordNotFound();
    }
    return res.success({ data:updatedPatient });
...
const deletePatient = async (req,res) => {
  try { 
    ...
    const query = { _id:req.params.id };
    const deletedPatient = await dbService.deleteOne(Patient, query);
    if (!deletedPatient){
      return res.recordNotFound();
    }
...
const deleteManyPatient = async (req, res) => {
  try {
    ...
    const deletedPatient = await dbService.deleteMany(Patient,query);
    if (!deletedPatient){
      return res.recordNotFound();
    }
    return res.success({ data :{ count :deletedPatient } });
  } catch (error){
    return res.internalServerError({ message:error.message }); 
  }
};
...
const softDeleteManyPatient = async (req,res) => {
  try {
    ...
    let updatedPatient = await dbService.updateMany(Patient,query, updateBody);
    if (!updatedPatient) {
      return res.recordNotFound();
    }
    return res.success({ data:{ count :updatedPatient } });
